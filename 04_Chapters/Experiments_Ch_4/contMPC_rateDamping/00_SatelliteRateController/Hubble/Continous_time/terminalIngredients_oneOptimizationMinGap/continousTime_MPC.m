...
V = -1 + 798507*x_1^4 + 206674*x_1^2*x_2^2 + 1.07768e+07*x_2^4 + 788510 ...
  *x_1^2*x_3^2 + 1.43488e+07*x_2^2*x_3^2 + 1.07768e+07*x_3^4;

P = 2.16431e+06*x_1^2 + 2.89352e+06*x_2^2 + 2.86663e+06*x_3^2;
...
for j = 1:numRuns
    ...
    x0_low = [0.0254;-0.0015; 0]; %x0_low_vec(:,1,j);
    x0     = x0_low;
    ...
    for k = 2:simTime/simStepSize
        ...
        if norm(x_sim(:,k)*180/pi,inf) < 1e-3 && norm(u_sol,inf) < 0.001
            break
        end
...
u_sol_vec_infMPC = u_sol_vec;
x_sol_vec_infMPC = x_sol_vec;
Q_infinite = Q;
R_infinite = R;
iter_conv_inf= k;
...
save('infiniteMPC_comparison.mat',...
