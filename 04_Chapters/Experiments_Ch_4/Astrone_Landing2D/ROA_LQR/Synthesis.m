%% ------------------------------------------------------------------------
%   
%
%   Short Description: Script to syntheszise the non-conflicting CBF/CLF for 
%                      a simple rendevouz problem. Only control constraints
%                      and simple box constraints are considered.
%
%
%   Needed software: - CasADi 3.6 (for CaSoS)
%                    - CaSoS
%
%
% ------------------------------------------------------------------------

clear
close all
clc

% system states
x = casos.PS('x',6,1);
u = casos.PS('u',2,1);

%% Astrone Parameter
m = 150;
g = 1.67e-4; 
lambda = 0.41; 
% control constraint; assumption is that the box is inside the full
% torque volume. This is roughly estimated visually.

umax = [30*1e-3;30*1e-3];
umin = 0.01*umax;


% scaling matrix for system states
Dx = eye(6);
Dxin = inv(Dx);


n = 2;
g0 = (x(1)^2/10^2)^(n/2) + (x(2)^2/10^2)^(n/2) + (x(3)^2/0.1^2)^(n/2)+ ...
     (x(4)^2/0.1^2)^(n/2) + (x(5)^2/0.3491^2)^(n/2) + (x(6)^2/0.0349^2)^(n/2)-1 ;

Dx   = diag([0.1;0.1;10;10;2.8648;28.6479]);
% Dx   = eye(6);

%% system dynamics

% Taylor-series approximation for trigonometric functions
sin_app = @(z) z-1/6*z^3;
cos_app = @(z) 1-1/2*z^2;
     
f = [x(3:4); 
    -(u(1)+u(2))*[sin_app(x(5));cos_app(x(5))]-[0;g]; 
    x(6); 
    lambda*(u(2)-u(1))];


x0 = [0;0;0;0;0;0];
u0 = [m*g/2;m*g/2];

A = full(casos.PD(subs(nabla(f,x),[x;u],[x0;u0])));
B =  full(casos.PD(subs(nabla(f,u),[x;u],[x0;u0])));

% cost function weights
Q = diag([1, 1, 1, 1, 1 ,1]);
R = eye(2);

% generate an initial guess for CLF
[K0,P0] = lqr(full(A),full(B),Q,R);

% scaled initial guess for CLF
Vval = (inv(Dx)*x)'*P0*(inv(Dx)*x);

% shift dynamics
f = subs(f,[x;u],[x+x0;u+u0]);

% scale dynamics
f = Dx*subs(f,[x;u],[Dx\x;u]);

% re-scale input of state constraints
g = subs(g0,x,Dx\x); 

%% setup SOS problem
% CLF
V  = casos.PS.sym('v',monomials(x,2)); 

% SOS mulitplier
s1 = casos.PS.sym('s1',monomials(x,2)); %
K  = casos.PS.sym('K',monomials(x,1),[2 1]);
s2 = casos.PS.sym('s2',monomials(x,2));
s3 = casos.PS.sym('s3',monomials(x,0),[2 1]);
s4 = casos.PS.sym('s4',monomials(x,0),[2 1]);
b  = casos.PS.sym('b');

% options for sequential sos
opts = struct('sossol','mosek');
opts.error_on_fail = 0;
opts.verbose       = 1;

b = 1;
sos = struct('x', [s2;V;K;s3;s4],...  % decision variables
             'f',dot(g-(V),g-(V)),...
              'p',[]);                             % parameter


% constraints
sos.('g') = [ 
             s2;
             s3;
             s4;
             s2*(V-b) - g;  
             % s3*(V-b) + umax - K
             % s4*(V-b) +  K - umin
             ];

% states + constraint are linear/SOS cones
opts.Kx = struct('lin', length(sos.x));
opts.Kc = struct('sos', length(sos.g));

% solver setu
S  = casos.nlsossol('S','sequential',sos,opts);

x0 =casos.PD([ x'*x;g; ones(2,1)*(x'*x); ones(4,1)]);

%  solve
sol = S('x0',x0);

bsol = b;

% re-scale invariant set, terminal penalty and local control law
Vsol_re = subs(sol.x(2),x,Dx*x) - full(casos.PD(bsol)); % write CBF as sublevel set 


%% plotting
import casos.toolboxes.sosopt.*

% slice for rates, i.e. sigma = [0,0,0]' satellite is aligned with inertial
% frame
figure(100)
pcontour(subs(Vsol_re,x(3:6),0),0,[-1 1 -1 1]*10,'k-')
hold on
pcontour(subs(g0,x(3:6),0),0,[-1 1 -1 1]*10,'r--')
