...
#else
  #define CASADI_PREFIX(ID) suffCon_mex_ ## ID
#endif
...
void mex_f(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  if (--argc>=0) arg[1] = casadi_from_mex(argv[1], w+3, casadi_s0, w+10);
  if (--argc>=0) arg[2] = casadi_from_mex(argv[2], w+6, casadi_s0, w+10);
  --resc;
  res[0] = w+9;
  f_incref();
  mem = f_checkout();
  i = f(arg, res, iw, w+10, mem);
