...
estValFun = (nabla(P,x)*subs(f,u,K) + (inv(Dx)*x)'*Q*(inv(Dx)*x) + K'*R*K);

sos = struct('x',[V; P; K; s1; s2; s3; s5;s8],...
              'f',dot( g-V, g-V ) + dot(estValFun,estValFun),... 
              'p',[]);
...
[coeffGuess,monoGuess] = poly2basis(sol.x);
...
penalties = full(penalty(samples(1,idx),samples(2,idx),samples(3,idx)));

% visualize samples of penalty
figure(100)
histogram(penalties)
...
max_dist = max(penalties.*penalties)
