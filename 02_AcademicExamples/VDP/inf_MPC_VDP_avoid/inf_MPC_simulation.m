...
Wfun_c = Function('f',{x,u},{jacobian(W,x)*xdot + gamma*W});   
...
for i = 1:2
    ...
    xlabel('t [s]');
    ylabel(sprintf('x_%c', num2str(i-1)));
    grid on;
...
% system states
x = casos.PS('x',2,1);
% g =     -1 - 5.2275*x(1)^2 + 49.2702*x(1)*x(2) - 5.22752*x(2)^2 + 15.8704*x(1)^3 ...
%   + 57.3519*x(1)^2*x(2) + 57.352*x(1)*x(2)^2 + 15.8705*x(2)^3 + 22.1005*x(1)^4 ...
%   + 7.98149*x(1)^3*x(2) + 63.9657*x(1)^2*x(2)^2 + 7.98156*x(1)*x(2)^3 + 22.1006...
%   *x(2)^4;
% g = -0.1 + 14.4701*x(1)^2 + 78.8666*x(1)*x(2) + 14.4701*x(2)^2 - 33.0108*x(1)^3   ...
%   - 113.724*x(1)^2*x(2) - 113.724*x(1)*x(2)^2 - 33.0108*x(2)^3 + 18.949*x(1)^4   ...
%   + 33.1438*x(1)^3*x(2) + 104.037*x(1)^2*x(2)^2 + 33.1438*x(1)*x(2)^3 + 18.949 ...
%   *x(2)^4;
...
for i = 1
    ...
    for j = 1:numRuns
        ...
        stairs(t(1:iter_all{j}-1), u_sol_all{j}(i,(1:iter_all{j}-1)), 'Color', colors(j, :));
    end
    xlabel('t [s]');
