...
sos.g = [
        ...
        s4;
        V - 1e-6*(x'*x);                           % V > 0
        s1*(V - b) - nabla(V,x)*f_cl - 1e-6*(x'*x)               % decay inside V<=b
        s2*(V - b) + K - umin_shift;
        s3*(V - b) + umax_shift - K;
        s4*(V - b) - g_shift;
];
...
pcontour(subs(Vsol_re,x,x-xeq),0,[-1 1 -1 1],'g'); hold on
...
if full(Vfun(x_sim_shift(1,1),x_sim_shift(2,1))-full(PD(b))) > 0
