...
% dynamics in shifted systems for QP
xdot = f_shifted;

% control bounds  shifted by trim
u_low = umin-ueq;
u_up  = umax-ueq;
...
Wfun_c = Function('f',{x,u},{jacobian(W,x)*xdot(u,x(1),x(2)) + gamma*W});   
...
% terminal penalty
Phi = Function('f',{x,u},{jacobian(V,x)*xdot(u,x(1),x(2))});

%% fixed-step Runge-Kutta 4 integration for simulation (using original dynamics)
f = Function('f', {x, u}, {xdot(u,x(1),x(2))});
...
%% setup QP solver in shifted-coordinates
...
% initial conditions in real coordinates
a4 = -1;  b4 = 1;
x0_low = (b4-a4)*rand(2,1000)+a4;

% we only consider initial states that lie in the terminal set; CBF in
% shifted coordinates!
idx = full(Wfun(x0_low-xeq)) <= 0;
...
for j = 1:numRuns
    ...
    % get initial conditions for the j-th run in real-coordinates
    x0 = x0_low(:,j); 
    ...
    % set initial conditions  in real coordinates
    x_sim                  = x0;
    ...
    % barrier in shifted coordinates
    Barrier(1) =     full(Wfun( x_sim(:,1) - xeq ) );

    for k = 2:simTime/simStepSize

        % Solve continous-time infinetismal-horizon MPC
        [sol] = solver('x0', z0, ...
                        'p', x0- xeq, ... % shifted coordinate
                        'lbx', lbz, ...
                        'ubx', ubz, ...
                        'lbg', lbg, ...
                        'ubg', ubg);
        ...
        % Extract solution of QP; soultion is 
        u_sol = full(sol.x);

        % store for later plotting
        u_sol_vec_infMPC(:,k-1) = ueq + u_sol;
        
        % Simulate in unshifted coordinates
        x_sim(:,k)      = sim_mex(x_sim(:,k-1), u_sol_vec_infMPC(:,k-1));
        
        % store solution for later plotting in real-coordinates
        x_sol_vec_infMPC(:,k)  = x_sim(:,k);
        
        % Store sufficient condition i.e. cost of QP (cost in shifted
        % coordaintes)
        suffCon(k-1) = full(sol.f);
        
        % barrier is in shifted coordinates
        Barrier(k) =     full(Wfun( x_sim(:,k) - xeq ));

        % check convergence 
        if norm(x_sol_vec_infMPC(:,k)-xeq,inf) < 1e-4      
           break
        end
