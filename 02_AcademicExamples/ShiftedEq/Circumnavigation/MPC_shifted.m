...
x0_real = [[-10;-25;-5]*1000;zeros(3,1)];
...
R = diag([1,1,1]);               % input cost

%% Define equilibrium point (not at origin)
v0 = zeros(3,1);        % meter per second    
r0 = [3;7;9]*1000; % meter
x_e = [r0;v0];       % equilibrium state
u_e = cpm(omega)^2*r0-g;        % equilibrium input (steady-state control)
...
xdot_real = [ x(4:6); u+g-2*cpm(omega)*x(4:6)-cpm(omega)^2*x(1:3)];

%% Shift system dynamics to error coordinates
% In shifted coordinates: x̃ = x - x_e, ũ = u - u_e
x_shifted = SX.sym('x_shifted',6,1);
u_shifted = SX.sym('u_shifted',3,1);
...
A_shift = full(casadi.DM(substitute(jacobian(xdot_shifted,x_shifted),...
             [x_shifted;u_shifted],[zeros(6,1);zeros(3,1)])));
...
h0_real = 1 - (x(1)/a)^2 + (x(2)/b)^2 + (x(3)/c)^2;
...
x_lower_real = [-inf; -inf;-inf; -inf;-inf; -inf];
x_upper_real = [ inf;  inf; inf;  inf; inf;  inf];
...
z0 = zeros(N*(length(x)+length(u)),1); % initial guess
